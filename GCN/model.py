...
class Generator(nn.Module):
    ...
    def __init__(self, window_size, node_num, in_features, out_features, lstm_features):
        ...
        self.lstm = nn.LSTM(
            input_size=out_features * node_num*6,
            hidden_size=lstm_features,
            num_layers=1,
            batch_first=True
        )
        self.ffn = nn.Sequential(
            nn.Linear(lstm_features, node_num * node_num*6),
            nn.Sigmoid()
        )
    ...
    def forward(self, in_shots):
        ...
        eye = torch.eye(node_num).cuda().unsqueeze(0).unsqueeze(0).expand(batch_size, window_size, node_num, node_num, 6)
